...
export const MySales: React.FC<Props> = props => {
      ...
      {props.sales.length <= 0 ? (
        ...
        <TableContainer>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell align='left'>購入されたレビュー</TableCell>
                <TableCell align='left'>購入ユーザー</TableCell>
                <TableCell align='left'>日付</TableCell>
                <TableCell align='left'>価格</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {props.sales.map((salesItem: SaleTypes) => (
                <TableRow key={uuidv4()}>
                  <TableCell
                    align='left'
                    style={{
                  ...
                  >
                    <Button
                      onClick={() =>
                        showContentsDialog(
                          salesItem.reviewed_contents,
                          salesItem.purchaser_profile,
                          salesItem.date
                        )
                      }
                      color='primary'
                    >
                      {salesItem.reviewed_contents.review.title}
                    </Button>
                  </TableCell>
                  ...
                  <TableCell align='left' style={{ width: 100, maxWidth: 100 }}>
                    <Price color={'#EC576B'} text={`¥${salesItem.price}`} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
